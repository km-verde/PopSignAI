...
public class creatorBall : MonoBehaviour
{
    ...
    void ProcessGameDataFromString(string mapText)
    {
        ...
        foreach (string line in lines)
        {
            ...
            else if (line.StartsWith("FREEZELEVEL "))
            {
                string modeString = line.Replace("FREEZELEVEL", string.Empty).Trim();
                LevelData.isFreezeMode = int.Parse(modeString) == 0 ? false : true;
            }
            ...
            if (table.Count > 0)
            {
                ...
                else
                {
                    ...
                    while (distCovered < 1)
                    {
                        ...
                        Meshes.transform.position = Vector3.Lerp(startPos, targetPos, distCovered);
                        yield return new WaitForEndOfFrame();
                        if (startPos.y > targetPos.y && LevelData.isFreezeMode)
                        {
